...
${activeCharacter.appearance ? `Appearance: ${activeCharacter.appearance}\n` : ''}${activeCharacter.outfit ? `Current outfit: ${activeCharacter.outfit}\n` : ''}Skills: ${Object.entries(activeCharacter.skills || {})
  ...
  const charactersBlock = characters.length
    ? `OTHER CHARACTERS\n${characters.map(c => {
        ...
        if (seenRecently) {
          // Current outfit only for a character actually in play right now
          // (same gate as appearance/location right below) -- not for
          // every character ever met in the save, which would grow this
          // block without bound over a long game exactly the way the
          // Retriever (see roles/retriever.js) exists to prevent for facts.
