...
- Nothing reads this yet — it exists so a future consistency check can catch
  a chapter that describes a month having passed, then a later chapter
  treating something from that point as "this morning" (a real bug this app
  hit once). Fill it honestly every turn regardless of whether anything
  reads it today.
...
Skills: ${Object.entries(activeCharacter.skills || {})
  ...
  // with this character's name) are appended either way, independent of the
  // recent-turns window and capped to the same RELEVANT_FACTS_LIMIT as the
  // general fact pool (see gatherTurnContext) -- this is what keeps a fact
  // like "Edgar has served 30 years" correct no matter how many turns pass
  // without anyone restating it, while still keeping the prompt bounded as a
  // long-lived character accumulates facts across a very long save.
...
function buildArchivistMasterPrompt({ language }) {
  const languageName = LANGUAGE_NAMES[language] || LANGUAGE_NAMES.fr;
...
{
  "new_facts": [{ "fact": string, "character": string | null, "type": "biographical" | "plot" }]
}
...
function buildArchivistPrompt({ characterNames, playerAction, chapterText, language }) {
...
${chapterText}${namesHint}`;
  ...
  return { system: buildArchivistMasterPrompt({ language }), user };
}
