  ...
  "playable_characters": [   // 3-4 distinct characters the player can choose to play as
    {
      ...
      "appearance": string,  // concrete physical description: age range, sex/gender presentation,
                             // ethnicity or skin tone, build, hair, and any distinguishing features
                             // (scars, clothing style, etc.) — never left vague or skipped. This is
                             // the only field the portrait image model actually sees, and it's read
                             // back into every turn's prompt so the narrator stays consistent about who
                             // this character is — a missing or generic appearance here is exactly what
                             // causes a character's portrait or described looks to drift or contradict
                             // themselves later.
  ...
  "starting_characters": [   // NPCs the player may encounter, not playable
    {
      ...
      "appearance": string,   // concrete physical description, same standard as playable_characters
                               // above (age range, sex/gender presentation, ethnicity or skin tone,
                               // build, hair, distinguishing features) — never vague or skipped, so the
                               // narrator describes this NPC the same way every time they appear.
...
${activeCharacter.appearance ? `Appearance: ${activeCharacter.appearance}\n` : ''}${activeCharacter.outfit ? `Current outfit: ${activeCharacter.outfit}\n` : ''}Skills: ${Object.entries(activeCharacter.skills || {})
  ...
  const charactersBlock = characters.length
    ? `OTHER CHARACTERS\n${characters.map(c => {
        ...
        // Current outfit shown either way, seen-recently or not -- unlike
        // appearance/location (dropped for a terser entry to keep the
        // prompt bounded), this is exactly the kind of thing worth the few
        // extra tokens: a disguise or a change of clothes from several
        // turns ago is precisely what a terse entry would otherwise forget.
        const outfitLine = c.outfit ? `Current outfit: ${c.outfit}` : null;
        if (seenRecently) {
          const details = [c.status, c.appearance ? `Appearance: ${c.appearance}` : null, outfitLine, c.location ? `Usually at: ${c.location}` : null]
            .filter(Boolean).join(' | ');
          return `- ${c.name} (${c.role || 'unknown role'}): ${details}${factLines ? `\n${factLines}` : ''}`;
        }
        const terse = [c.oneLiner || c.status, outfitLine].filter(Boolean).join(' | ');
        return `- ${c.name}: ${terse}${factLines ? `\n${factLines}` : ''}`;
      }).join('\n')}`
    : 'OTHER CHARACTERS\n(none introduced yet)';
...
- Follow CHARACTER'S BEHAVIOR DURING THE SKIP below (if given) for how the
  player character actually spends this stretch of time — this is
  distinct from PLAYER'S REQUEST above: the request is what the skip
  should lead to or cover, this is what the character is doing/how they're
  behaving while it happens (e.g. training cautiously, staying withdrawn,
  actively making contacts). Let it actually shape the montage rather than
...
function buildTimeSkipPrompt({
  ...
  storyClock, timelineEvents, hint, behavior, language, chapterLength
}) {
  ...
  // Distinct from requestBlock above: what the skip should lead to/cover
  // vs. how the character actually spends it -- e.g. "until I reach the
  // capital" (request) vs. "staying cautious, avoiding attention" (this).
  // Optional -- most skips won't need it spelled out separately from the request.
  const behaviorBlock = (behavior && behavior.trim())
    ? `CHARACTER'S BEHAVIOR DURING THE SKIP: ${behavior.trim()}`
    : null;
...

{
  ...
  "outfit_changes": [{ "character": string, "description": string, "reason": string | null }]
}
...
CLOTHING & DISGUISES: separate from NEW FACTS above -- an outfit is
*current* state (what someone is wearing right now), not an accumulating
fact, so this replaces whatever that character was wearing before rather
than adding to a list.
- "character": the exact name of who changed what they're wearing, matching
  one of the names given below (never null -- an outfit is always someone's).
- "description": a full, self-contained description of everything they're
  now wearing, written so it could stand alone as a portrait/scene prompt
  without needing anything said before it (not a diff like "put on a
  cloak" -- restate the whole outfit, cloak included, even the parts that
  didn't change).
- "reason": why, if the chapter makes it explicit and it matters for
  understanding the scene (a disguise to sneak past someone, changing out
  of wet clothes, dressing for a formal occasion) -- null if it's just an
  ordinary change with no narrative weight, or if the chapter doesn't say.
- Only report an entry when the chapter actually shows this character
  putting something on, taking something off, or otherwise ending up in a
  visibly different outfit than before -- not every time someone's clothing
  is merely mentioned or described again unchanged. See CURRENT OUTFITS
  below for what's already on record; if nothing changed for a character,
  leave them out entirely.
- Empty array if nobody's clothing changed this chapter.
...
function buildArchivistPrompt({ characterNames, playerAction, chapterText, currentOutfits, povCharacter, language }) {
  const namesHint = (characterNames && characterNames.length)
    ...
    : '';
  // What's already on record per character (see lib/characterOutfits.js) --
  // without this the model has no way to tell "still wearing the same
  // thing" from "changed", and CLOTHING & DISGUISES above explicitly asks
  // it to only report real changes.
  const outfitsBlock = (currentOutfits && Object.keys(currentOutfits).length)
    ? `\n\nCURRENT OUTFITS ON RECORD (only report a change if this chapter shows one of these no longer applying)\n${Object.entries(currentOutfits).map(([name, outfit]) => `- ${name}: ${outfit}`).join('\n')}`
    : '';
...
${chapterText}${namesHint}${outfitsBlock}`;
...
// Migration plan, Milestones 3-4). Milestone 3 shipped a narrow first pass
// (pacing language against storyClock only, the exact scenario that
// motivated the doc: a month passing then a chapter casually calling it
// "this morning"). This is that check PLUS Milestone 4's full pass: the
// chapter is also checked against the facts most relevant to it (similarity
// retrieved the same bounded way the Writer's own KNOWN FACTS block is --
// see gameEngine.js's fireProofreader -- not a literal dump of the entire
// archive, which would cost real money and latency every single turn for
// diminishing return on a long save). Crucially, that retrieval is
// omniscient (see lib/db.js's searchMemoryFactsByEmbedding): a private fact
// from a POV scene or a secret the player character was never told is
// exactly the kind of thing this check most needs to see, since it's
// checking the STORY's consistency with itself, not what any one character
// is allowed to know.
//
// Fired the same fire-and-forget way as the Archivist (see
...
function buildProofreaderMasterPrompt({ language }) {
  const languageName = LANGUAGE_NAMES[language] || LANGUAGE_NAMES.fr;
  return `You are a continuity checker for a choose-your-own-adventure game.
You are given a single already-written chapter, the story clock, and the
facts on record most relevant to this chapter (including ones the player
character themselves may not know -- you are checking the story's
consistency with itself, not with what any one character is aware of). You
do not write narrative and you do not judge success or failure -- you check
two, and only two, things:

1. PACING: is THIS CHAPTER'S OWN pacing language (words and phrases like
   "this morning", "just now", "moments later", "still", vs. "weeks later",
   "since then", "by now") consistent with the story clock below it? The
   failure mode: a stretch of time was just established (a time skip, an
   elapsed description, a new date) and the chapter's own wording still
   treats the scene as continuing immediately from whatever came right
   before it.
2. FACTS: does anything the chapter states outright contradict one of the
   facts on record below -- a concrete detail (a name, a number, an exact
   date, a stated relationship, a described appearance, something someone
   was told or promised) that could not both be true?

You do not check anything beyond these two: not writing quality, not
whether the plot makes sense, not whether a character's reaction feels
right -- only pacing against the story clock and stated details against the
facts on record.
...
or, with one entry per real issue found (there can be more than one):

{ "contradictions": [{ "type": "pacing" | "fact", "summary": string, "quote": string }] }

- Use an empty array for the overwhelming majority of chapters -- most
  raise no pacing claim and touch no fact on record at all, or are
  perfectly consistent with both, and all of those mean [].
- Only flag a REAL, CONCRETE contradiction — specific wording in the
  chapter that could not both be true. Do not flag stylistic looseness,
  vague language, plausible new detail that simply hasn't been stated
  before, or anything you're not confident is a genuine mismatch. A false
  positive here is worse than a missed one: the author acts on every flag
  raised.
- "type": "pacing" for issue 1 above, "fact" for issue 2.
- "summary": one short sentence, in ${languageName}, explaining the
  contradiction in plain terms.
- "quote": the exact phrase from the chapter below that conflicts, copied
  verbatim.`;
}

function buildProofreaderPrompt({ chapterText, storyClock, relevantFacts, language }) {
  ...
  const factsBlock = (relevantFacts && relevantFacts.length)
    ? `FACTS ON RECORD (may include things the player character themselves doesn't know -- check the chapter against all of them)\n${relevantFacts.map(f => `- ${f}`).join('\n')}`
    : 'FACTS ON RECORD\n(nothing on record yet, or nothing similar enough to this chapter to check against)';
...
- If a previous plan is given: keep whatever of it the recent scenes haven't
  ...
  -- don't invent an unrelated new direction every time you're asked. You
  are asked this every single turn now, not periodically -- most turns
  change nothing worth changing, and returning the previous plan almost
  verbatim is the right answer far more often than not; only revise a beat
  when the player's last action genuinely made it stale (they killed the
  NPC it depended on, found out the secret it was building toward, etc.).
...

{
  ...
  "appearance": string,   // concrete physical description: age range, sex/gender presentation,
                          // ethnicity or skin tone, build, hair, and any distinguishing features —
                          // never left vague or skipped. This is what the portrait image model
                          // actually sees, and it's read back into every turn so the narrator
                          // describes this character consistently from here on.
