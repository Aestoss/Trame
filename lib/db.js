...
function searchMemoryFactsByEmbedding({ saveId, embedding, k, character }) {
  // status/character/type live only inside `data` (see TABLE_COLUMNS --
  // memoryFacts only indexes id/saveId/turnNumber), so the candidate pool
  // is narrowed by the real saveId index first, then filtered in JS --
  // same pattern splitMatcher's jsKeys fallback already uses elsewhere.
  ...
  const candidates = rows
    ...
    .filter(({ fact }) => fact.status === 'active' && (character ? fact.character === character : (!fact.character && fact.type !== 'biographical')));
