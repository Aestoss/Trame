...
function fireArchivist({ world, saveId, turnNumber, playerAction, chapterText, characters, settings, povCharacter }) {
  archivist.extractFacts({
    worldId: world.id, saveId, turnNumber, playerAction, chapterText,
    characterNames: characters.map(c => c.name), povCharacter, settings
...
async function playPovTurnStreaming(saveId, povCharacterName, hint, { providerOverride, onChapterChunk } = {}) {
  ...
  const viewpointCharacter = { ...povRow, facts: factsByCharacter[povRow.name] || [] };
  const otherCharacters = allCharacterRows
    .filter(c => c.name !== povCharacterName)
    .map(c => ({ ...c, facts: factsByCharacter[c.name] || [] }));
  const mcFacts = factsByCharacter[mcCharacter.name] || [];
  if (mcFacts.length) {
    otherCharacters.push({ name: mcCharacter.name, role: '', status: '', oneLiner: '', appearance: '', location: '', facts: mcFacts });
  }
  ...
  fireArchivist({ world, saveId, turnNumber, playerAction, chapterText: turn.chapterText, characters: otherCharacters, settings, povCharacter: povCharacterName });
  return turn;
}
...
module.exports = {
  ...
  getStoryClock, getActiveMastermindPlan,
