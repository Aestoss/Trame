...
const { getCurrentOutfit, applyOutfitChange, getOutfitHistory } = require('./characterOutfits');
const archivist = require('../roles/archivist');
const retriever = require('../roles/retriever');
const proofreader = require('../roles/proofreader');
const mastermind = require('../roles/mastermind');
const {
  ...
  buildTimeSkipPrompt, buildPovPrompt
} = require('./promptBuilder');

const RECENT_TURNS_WINDOW = 5;
const SUMMARIZE_EVERY = 10;
const RELEVANT_FACTS_LIMIT = 20;
// Milestone 3 originally gated the Mastermind to every 5th turn; per
// feedback item #1 it now fires every turn (still async/fire-and-forget --
// see fireMastermind), so its plan reacts to what the player just did
// instead of being up to 4 turns stale.
...
async function callText({ system, user, kind, worldId, saveId, providerOverride }) {
  ...
  // One line per call, structural metadata only (never prompt/response
  // content) -- lets Railway's own deploy logs answer "which provider/model
  // actually served this?" without a debug dump, e.g. confirming a fallback
  // actually took over, or that a provider override was honored.
  console.log(`[writer] kind=${kind} save=${saveId || '-'} provider=${provider} model=${model || '(default)'}${providerOverride ? ' (override)' : ''}`);
  return text;
}
...
async function streamTextTracked({ system, user, kind, worldId, saveId, onDelta }) {
  ...
  recordCost({ worldId, saveId, kind, provider, model, usage });
  console.log(`[writer] kind=${kind} save=${saveId || '-'} provider=${provider} model=${model || '(default)'} (streamed)`);
  return text;
}
...
async function createWorld(playerIdea, language, { onChunk } = {}) {
  ...
  const playableCharacters = (parsed.playable_characters || []).map(c => ({
    id: uuid(), worldId: world.id, name: c.name, description: c.description, appearance: c.appearance || '',
    skills: c.skills || {},
...
// appearance (the concrete physical description) drives the portrait --
// description is background/personality, not necessarily anything visual --
// falling back to description only for characters saved before appearance
// existed.
...
function addCharacter(worldId, { name, description, appearance, skills, initialTrackedItemValues }) {
  const world = getWorld(worldId);
...
function updateCharacter(worldId, characterId, { name, description, appearance, skills, initialTrackedItemValues }) {
  ...
  const next = {
    ...
    appearance: appearance !== undefined ? appearance : character.appearance,
...
// Author-mode-only troubleshooting tool: dumps everything about a save's
// current state to the deploy log in one line, single-line JSON so a log
// shipper (Railway included) never splits it across multiple entries. The
// ongoing per-call [writer]/[archivist]/[proofreader]/[mastermind] logs
// (see callText/streamTextTracked above and roles/*.js) already answer "is
// this role running" as it happens; this is for "show me everything about
// THIS save right now" -- recent facts, the Mastermind's actual plan
// (never surfaced anywhere else, by design), every Proofreader flag, and a
// per-role cost breakdown, all in one place instead of five separate
// queries nobody can actually run against the live database.
function logDebugSnapshot(saveId) {
  const save = getSave(saveId);
  const world = getWorld(save.worldId);
  const settings = getSettings();
  const turns = db.get('turns').filter({ saveId }).sortBy('turnNumber').value();
  const facts = db.get('memoryFacts').filter({ saveId }).value();
  const mastermindPlans = db.get('mastermindPlans').filter({ saveId }).sortBy('turnNumber').value();
  const activePlan = mastermindPlans.find(p => p.status === 'active');
  const proofreaderFlags = db.get('proofreaderFlags').filter({ saveId }).value();
  const timelineEvents = db.get('timelineEvents').filter({ saveId }).value();
  const storyClock = db.get('storyClock').find({ saveId }).value();
  // Full tracked-item picture, defs + this save's current values side by
  // side -- added after a report of an item ("Power Level") that seemed to
  // vanish from play despite still being listed in the World Editor. The
  // routine per-turn display only ever shows the *current* def list
  // filtered by visibility (see server.js's publicTurn), which gives no way
  // to tell a visibility flip from a genuinely missing value row from a
  // worldId mismatch just by looking at the story -- this lays all three
  // out explicitly.
  ...
  const trackedItemsDebug = itemDefs.map(def => {
    const valueRow = itemValues.find(v => v.itemDefId === def.id);
    return {
      name: def.name, defWorldId: def.worldId, visibility: def.visibility,
      hasValueRowForThisSave: Boolean(valueRow),
      value: valueRow ? valueRow.value : def.initialValue
    };
  });

  // Current + past outfits per character (see lib/characterOutfits.js) --
  // added alongside the tracked-items detail above for the same reason: a
  // report of clothing/disguise coherence breaking down needs to show
  // whether anything was ever recorded for a character, not just the
  // Writer's own (unverifiable) chapter text.
  const allOutfits = db.get('characterOutfits').filter({ saveId }).sortBy('startTurn').value();
  const outfitsByCharacter = {};
  for (const o of allOutfits) {
    (outfitsByCharacter[o.character] || (outfitsByCharacter[o.character] = [])).push({
      description: o.description, reason: o.reason, startTurn: o.startTurn, endTurn: o.endTurn
    });
  }

  const costByKind = {};
  for (const row of db.get('costLog').filter({ saveId }).value()) {
    const k = row.kind || 'unknown';
    const bucket = costByKind[k] || (costByKind[k] = { calls: 0, inputTokens: 0, outputTokens: 0, estimatedCostUsd: 0 });
    bucket.calls++;
  ...
  const snapshot = {
    saveId, worldId: world.id, worldTitle: world.title, turnCount: turns.length,
    activeCharacterId: save.activeCharacterId, gameOver: save.gameOver,
    settings: {
      textProvider: settings.textProvider, textModel: settings.textModel || '(default)',
      backgroundModelKeySet: Boolean(settings.backgroundModel && settings.backgroundModel.apiKey),
      embeddingProvider: settings.embeddingProvider,
      imageProvider: settings.imageProvider, imagesEnabled: settings.imagesEnabled
    },
    storyClock: storyClock ? { currentDate: storyClock.currentDate, elapsedDescription: storyClock.elapsedDescription } : null,
    timelineEventCount: timelineEvents.length,
    recentTurns: turns.slice(-5).map(t => ({
      turnNumber: t.turnNumber, playerAction: t.playerAction, outcome: t.outcome,
      povCharacter: t.povCharacter, chapterPreview: (t.chapterText || '').slice(0, 140),
      // Which tracked items this turn's own snapshot actually has a value
      // row for -- pinpoints the turn a given item's row stopped being
      // carried forward, as distinct from the *current* itemDefs/values
      // picture below (trackedItemsDebug), which only reflects right now.
      snapshotTrackedItemNames: ((t.snapshot && t.snapshot.trackedItemValues) || [])
        .map(v => (itemDefs.find(d => d.id === v.itemDefId) || {}).name || v.itemDefId)
    })),
    memoryFactsTotal: facts.length,
    recentFacts: facts.slice(-20).map(f => ({
      turnNumber: f.turnNumber, fact: f.fact, character: f.character, type: f.type, knownBy: f.knownBy, status: f.status
    })),
    mastermindPlanHistoryCount: mastermindPlans.length,
    mastermindActivePlan: activePlan ? { turnNumber: activePlan.turnNumber, summary: activePlan.summary, beats: activePlan.beats } : null,
    proofreaderFlags,
    trackedItemsDebug,
    outfitsByCharacter,
    costByKind
  };
  console.log(`[debug-dump] save=${saveId} ${JSON.stringify(snapshot)}`);
}
...
function rewindToTurn(saveId, targetTurnNumber) {
  ...
  const snapshot = targetTurn.snapshot || { trackedItemValues: [], characters: [], secretInfo: '', currentLocation: '', gameOver: null };
  ...
  // Same for a Proofreader flag raised against a turn that's about to
  // disappear -- it was about a chapter the player will never read again.
  db.get('proofreaderFlags').remove(f => f.saveId === saveId && f.turnNumber > targetTurnNumber).write();
  // A Mastermind plan generated at/after the rewind point was written for
  // turns that no longer exist -- discarding it (rather than leaving it
  // active) keeps the Writer from being steered by a plan built for a
  // future that got undone. Plans from at or before the rewind point are
  // exactly what the guardrail in buildMastermindPrompt means by "already
  // established" -- they stay.
  db.get('mastermindPlans').remove(p => p.saveId === saveId && p.turnNumber > targetTurnNumber).write();
  // The plan that WAS active before the rewind is very likely one of the
  // rows just removed above (now that the Mastermind fires every turn --
  // feedback item #1 -- almost any rewind removes the current active plan),
  // which would otherwise leave the save with zero active plans until the
  // next turn happens to write a new one. Reactivate whichever remaining
  // plan has the highest turnNumber, if no plan is currently active.
  if (!getActiveMastermindPlan(saveId)) {
    const remainingPlans = db.get('mastermindPlans').filter({ saveId }).sortBy('turnNumber').value();
    const lastRemaining = remainingPlans[remainingPlans.length - 1];
    if (lastRemaining) {
      db.get('mastermindPlans').find({ id: lastRemaining.id }).assign({ status: 'active', supersededBy: null }).write();
    }
  }

  // Same "discard what's about to not exist" reasoning as mastermindPlans
  // above, applied to each character's outfit history (see
  // lib/characterOutfits.js): an outfit that started after the rewind point
  // never happened; one that was ended after the rewind point (by a later
  // change of clothes) needs to become current again, since whatever
  // superseded it is exactly what's being undone.
  db.get('characterOutfits').remove(o => o.saveId === saveId && o.startTurn > targetTurnNumber).write();
  db.get('characterOutfits').filter(o => o.saveId === saveId && o.endTurn !== null && o.endTurn > targetTurnNumber).value()
    .forEach(o => db.get('characterOutfits').find({ id: o.id }).assign({ endTurn: null }).write());
...
function deleteSave(saveId) {
  ...
  db.get('proofreaderFlags').remove({ saveId }).write();
  db.get('mastermindPlans').remove({ saveId }).write();
  db.get('characterOutfits').remove({ saveId }).write();
...
// The Mastermind's hidden plan (Milestone 3): mastermindPlans is versioned
// (status active/superseded, same pattern as memoryFacts) but only the
// current active row is ever read back into a prompt -- see
// gatherTurnContext and fireMastermind below.
function getActiveMastermindPlan(saveId) {
  return db.get('mastermindPlans').find({ saveId, status: 'active' }).value();
}
...
function persistTurn({ saveId, world, playerAction, parsed, framedImagePrompt, imageUrl, itemDefs, allTurns, povCharacter }) {
  ...
  const turn = {
    ...
    // Set only by playPovTurnStreaming -- lets the pagination UI badge
    // which character (if any) this chapter was narrated through, same
    // spirit as the time-skip badge (see timelineEvents).
    povCharacter: povCharacter || null,
    createdAt: new Date().toISOString()
  };
  db.get('turns').push(turn).write();
  console.log(`[turn] save=${saveId} turn=${turnNumber} outcome=${turn.outcome}${povCharacter ? ` pov=${povCharacter}` : ''}${gameOver ? ` gameOver=${gameOver.result}` : ''}`);
  ...
  if (typeof updates.location === 'string' && updates.location.trim()) saveUpdates.currentLocation = updates.location.trim();
...
async function gatherTurnContext(saveId, playerAction) {
  ...
  const activeCharacterRow = db.get('playableCharacters').find({ id: save.activeCharacterId }).value();
  // Current outfit (see lib/characterOutfits.js) -- read back into the
  // PLAYER CHARACTER block below (buildTurnContextBlocks), distinct from
  // "appearance" (permanent physical traits): this is what fixes the Writer
  // losing track of a disguise or a change of clothes once it stops being
  // the most recent thing said, since it's now always in context instead of
  // needing to be recalled from the general fact archive.
  const activeOutfit = getCurrentOutfit(saveId, activeCharacterRow.name);
  const activeCharacter = { ...activeCharacterRow, outfit: activeOutfit ? activeOutfit.description : null };
  ...
  // The Mastermind's hidden plan (Milestone 3): fetched here (rather than
  // just before the return, like storyClock/timelineEvents below) because
  // feedback item #1 needs its stored embedding as the retrieval call's
  // forward query, right below.
  const mastermindPlan = getActiveMastermindPlan(saveId);
  ...
  // viewerCharacter scopes retrieval to the MC's own knowledge (see the
  // POV mechanic's knownBy wall, lib/db.js's searchMemoryFactsByEmbedding)
  // -- a fact learned only during another character's POV scene, with the
  // MC explicitly absent, is excluded here even if it's topically similar
  // to this turn's action.
  const { memoryFacts, factsByCharacter } = await retriever.retrieveRelevantFacts({
    saveId, queryText, forwardEmbedding: mastermindPlan && mastermindPlan.embedding,
    characterNames: characterRows.map(c => c.name), viewerCharacter: activeCharacter.name, limit: RELEVANT_FACTS_LIMIT, settings
  });
  const characters = characterRows.map(c => {
    const outfit = getCurrentOutfit(saveId, c.name);
    return { ...c, facts: factsByCharacter[c.name] || [], outfit: outfit ? outfit.description : null };
  });
  ...
  return { save, world, settings, activeCharacter, memoryFacts, worldLoreFacts, characters, itemDefs, trackedItems, allTurns, recentTurns, storyClock, timelineEvents, mastermindPlan };
}
...
function fireArchivist({ world, saveId, turnNumber, playerAction, chapterText, characters, activeCharacter, settings, povCharacter }) {
  // The MC's own name was never included here before -- characters is
  // gatherTurnContext's "OTHER CHARACTERS" list, which never contains the
  // player character. Without it, the Archivist's "character" field had no
  // exact name to tag an MC fact or outfit change with (see
  // buildArchivistPrompt's namesHint). Folded in here rather than fixed at
  // every call site.
  const allNames = activeCharacter ? [activeCharacter.name, ...characters.map(c => c.name)] : characters.map(c => c.name);
  // What each of them is currently wearing, if anything's on record (see
  // lib/characterOutfits.js) -- the Archivist needs this to tell "still the
  // same outfit" from "this is a change" (see CLOTHING & DISGUISES). Scoped
  // to characters this chapter actually mentions, NOT the whole cast ever
  // met in the save -- a save with dozens of characters met over a long
  // game must never turn into a block that grows every turn regardless of
  // who's actually in the scene; that's exactly what the Retriever (see
  // roles/retriever.js) exists to prevent for facts, and outfits get the
  // same discipline.
  const currentOutfits = {};
  for (const name of allNames) {
    // The MC is the one exception to the mention-gate just below: every
    // chapter is narrated in second person ("you..."), so the MC's own
    // name essentially never appears in chapterText even though they're
    // always the one in the scene -- unlike an NPC, there's no name to
    // match on, so this can't rely on the same signal. A single fixed
    // entity, not a growing list, so no bound needed here the way there is
    // for the cast below.
    const isMc = activeCharacter && name === activeCharacter.name;
    if (!isMc && !chapterText.includes(name) && !playerAction.includes(name)) continue;
    const outfit = getCurrentOutfit(saveId, name);
    if (outfit) currentOutfits[name] = outfit.description;
  }
  archivist.extractFacts({
    worldId: world.id, saveId, turnNumber, playerAction, chapterText,
    characterNames: allNames, currentOutfits, povCharacter, settings
  }).catch(e => console.error(`[archivist] unhandled error (save ${saveId}, turn ${turnNumber}): ${e.message}`));
}

// Fires the Proofreader (see roles/proofreader.js), same fire-and-forget
// pattern and same non-critical-path guarantee as fireArchivist above.
// Reads storyClock AFTER persistTurn has already run (not the value
// gatherTurnContext captured before the turn was played) -- a story_clock
// update or time skip from THIS turn is exactly the kind of pacing change
// the Proofreader needs to check the chapter's own wording against.
//
// Only called from playTurn/playTurnStreaming -- an ordinary turn is
// exactly the scenario this whole role exists for: the Writer can decide
// on its own, mid-turn, to compress time (see the autonomous time-skip
// path) and then keep narrating as if nothing had passed. Not called from
// playTimeSkipStreaming (the player explicitly asked to skip ahead -- a
// long elapsed description there is the intended outcome, not a
// contradiction) or playPovTurnStreaming (a POV scene narrates a different
// moment/character off its own "CURRENT SITUATION" anchor, see
// buildPovPrompt, rather than advancing the main story clock at all).
// Milestone 4: also retrieves the facts most relevant to the just-written
// chapter, the same bounded similarity search the Writer's own KNOWN FACTS
// block uses, but omniscient (see lib/db.js's searchMemoryFactsByEmbedding)
// -- this is a narrator-level check against everything on record, not
// scoped to what any one character is allowed to know. Wrapped in an async
// IIFE rather than making fireProofreader itself async: the retrieval has
// to happen before checkChapter can be called, but the function still needs
// to return immediately and stay fire-and-forget at the call site, same as
// fireArchivist/fireMastermind -- a single .catch() on the whole thing
// covers a failure in either step.
function fireProofreader({ world, saveId, turnNumber, chapterText, settings }) {
  (async () => {
    const storyClock = getStoryClock(saveId);
    const characterNames = db.get('saveCharacters').filter({ saveId }).value().map(c => c.name);
    const { memoryFacts, factsByCharacter } = await retriever.retrieveRelevantFacts({
      saveId, queryText: chapterText, characterNames, omniscient: true, limit: RELEVANT_FACTS_LIMIT, settings
    });
    const relevantFacts = [...new Set([...memoryFacts.map(f => f.fact), ...Object.values(factsByCharacter).flat()])];
    await proofreader.checkChapter({
      worldId: world.id, saveId, turnNumber, chapterText, storyClock, relevantFacts, settings
    });
  })().catch(e => console.error(`[proofreader] unhandled error (save ${saveId}, turn ${turnNumber}): ${e.message}`));
}

// Fires the Mastermind (see roles/mastermind.js), same fire-and-forget
// pattern as fireArchivist/fireProofreader above. Per feedback item #1,
// this now runs every turn (previously gated to every 5th) so the plan
// reacts to what the player just did -- e.g. a plan beat built around an
// NPC the player kills gets revised before the next turn is even written,
// instead of staying stale for up to 4 more turns. Staying async (not
// blocking the Writer) was a deliberate call: the plan the Writer reads
// this turn is always the most recently *completed* review, one turn
// behind at most, which is cheaper than adding a blocking model call to
// every single turn for a freshness window smaller than how long a player
// takes to read a chapter and act.
//
// Only called from playTurn/playTurnStreaming, same reasoning as
// fireProofreader: an explicit time skip or a POV scene is each its own
// short-lived deviation, not the ongoing story the plan is meant to track.
function fireMastermind({ world, saveId, turnNumber, settings }) {
  const previousPlan = getActiveMastermindPlan(saveId);
  const recentTurns = db.get('turns').filter({ saveId }).sortBy('turnNumber').value().slice(-RECENT_TURNS_WINDOW);
  const storyClock = getStoryClock(saveId);
  mastermind.reviewPlan({
    worldId: world.id, saveId, turnNumber, world, previousPlan, recentTurns, storyClock, settings
  }).catch(e => console.error(`[mastermind] unhandled error (save ${saveId}, turn ${turnNumber}): ${e.message}`));
}

async function playTurn(saveId, playerAction, { authorMode, authorNote, providerOverride } = {}) {
  const { world, settings, activeCharacter, memoryFacts, worldLoreFacts, characters, itemDefs, trackedItems, allTurns, recentTurns, save, storyClock, timelineEvents, mastermindPlan } = await gatherTurnContext(saveId, playerAction);
  ...
  const { system, user } = buildTurnPrompt({
    ...
    storyClock, timelineEvents, mastermindPlan, language: world.language || settings.language, chapterLength: settings.chapterLength, authorMode, authorNote
  });
  ...
  fireArchivist({ world, saveId, turnNumber, playerAction, chapterText: turn.chapterText, characters, activeCharacter, settings });
  fireProofreader({ world, saveId, turnNumber, chapterText: turn.chapterText, settings });
  fireMastermind({ world, saveId, turnNumber, settings });
  return turn;
}
...
async function playTurnStreaming(saveId, playerAction, { authorMode, authorNote, providerOverride, onChapterChunk } = {}) {
  ...
  const narrationPrompt = buildNarrationPrompt({
    world, memoryFacts, worldLoreFacts, characters, recentTurns, playerAction, activeCharacter, trackedItems, secretInfo: save.secretInfo,
    storyClock, timelineEvents, mastermindPlan, language, chapterLength: settings.chapterLength, authorMode, authorNote
  });
...
// would.
async function playTimeSkipStreaming(saveId, hint, behavior, { providerOverride, onChapterChunk } = {}) {
  // Both feed the retrieval query text (see gatherTurnContext) as well as
  // the prompt below -- a behavior note ("stays cautious, avoids notice")
  // is exactly the kind of thing worth surfacing relevant facts against,
  // same as the request itself.
  const playerAction = [hint, behavior].filter(s => s && s.trim()).join(' — ') || '(time skip)';
  ...
  const narrationPrompt = buildTimeSkipPrompt({
    ...
    storyClock, timelineEvents, hint, behavior, language, chapterLength: settings.chapterLength
  });
...
// The POV mechanic: a self-contained, player/author-initiated interlude
// narrated through povCharacterName instead of the player character, who
// is not present. activeCharacterId on the save never changes -- this is a
// narration-only device, not a character switch -- so the very next turn
// goes back through playTurnStreaming as normal.
//
// The knowledge wall (see lib/memoryFacts.js's knownBy, lib/db.js's
// searchMemoryFactsByEmbedding) is what keeps this from blurring what
// different characters know into one omniscient pool: retrieval here is
// scoped to viewerCharacter: povCharacterName throughout, so this prompt
// only ever receives facts that character could plausibly know -- not the
// player character's own private facts, and not secretInfo (omitted from
// the narration prompt entirely -- see buildPovPrompt). Facts extracted
// afterward get knownBy set to whoever was actually in this scene (see
// buildArchivistPrompt's povCharacter param below), so the wall holds
// going forward too: the player character's next turn can't retrieve what
// they weren't there for.
async function playPovTurnStreaming(saveId, povCharacterName, hint, { providerOverride, onChapterChunk } = {}) {
  const save = getSave(saveId);
  const world = getWorld(save.worldId);
  const settings = getSettings();
  if (!save.activeCharacterId) throw new Error('Choose a character before playing');
  ...
  const mcCharacter = db.get('playableCharacters').find({ id: save.activeCharacterId }).value();
  const allCharacterRows = db.get('saveCharacters').filter({ saveId }).value();
  const povRow = allCharacterRows.find(c => c.name === povCharacterName);
  if (!povRow) throw new Error('Character not found');
  ...
  const sceneWindow = recentTurns.map(t => `${t.playerAction} ${t.chapterText}`).join(' ');
  const queryText = `${hintText} ${sceneWindow}`.trim() || povCharacterName;

  // Includes the player character's own name so a synthetic OTHER
  // CHARACTERS entry below can show whatever povCharacterName happens to
  // know about them (usually little to nothing) -- same retrieval
  // mechanism as any other character, just scoped by viewerCharacter.
  const characterNamesToQuery = [...allCharacterRows.map(c => c.name), mcCharacter.name];
  const { memoryFacts, factsByCharacter } = await retriever.retrieveRelevantFacts({
    saveId, queryText, characterNames: characterNamesToQuery, viewerCharacter: povCharacterName, limit: RELEVANT_FACTS_LIMIT, settings
  });

  const povOutfit = getCurrentOutfit(saveId, povRow.name);
  const viewpointCharacter = { ...povRow, facts: factsByCharacter[povRow.name] || [], outfit: povOutfit ? povOutfit.description : null };
  const otherCharacters = allCharacterRows
    .filter(c => c.name !== povCharacterName)
    .map(c => {
      const outfit = getCurrentOutfit(saveId, c.name);
      return { ...c, facts: factsByCharacter[c.name] || [], outfit: outfit ? outfit.description : null };
    });
  const mcFacts = factsByCharacter[mcCharacter.name] || [];
  if (mcFacts.length) {
    const mcOutfit = getCurrentOutfit(saveId, mcCharacter.name);
    otherCharacters.push({ name: mcCharacter.name, role: '', status: '', oneLiner: '', appearance: '', location: '', facts: mcFacts, outfit: mcOutfit ? mcOutfit.description : null });
  }

  const playerAction = hintText || `(POV: ${povCharacterName})`;
  const provider = providerOverride?.provider || settings.textProvider;
  const model = providerOverride ? providerOverride.model : settings.textModel;

  const narrationPrompt = buildPovPrompt({
    world, memoryFacts, worldLoreFacts, otherCharacters, recentTurns, viewpointCharacter,
    storyClock, timelineEvents, hint: hintText, language, chapterLength: settings.chapterLength
  });

  // Same chunk-forwarding logic as playTurnStreaming/playTimeSkipStreaming
  // -- see playTurnStreaming's own comment for why the end-marker holdback
  // is needed. Unchanged because the wire format is identical either way.
  let rawBuffer = '';
  let visibleSent = 0;
  let sawEndMarker = false;
  const forwardChapterChunk = (chunk) => {
  ...
  // Same state-extraction call as any other turn, with povScene: true so
  // it knows not to update the player character's own tracked items for a
  // scene they weren't in (see buildStateMasterPrompt's TRACKED ITEMS
  // section) -- secretInfo/story_clock/time_skip stay live, since those
  // are narrator-level state a POV scene can still meaningfully move.
  const statePrompt = buildStatePrompt({
    world, characters: otherCharacters, worldLoreFacts, activeCharacter: null, trackedItems,
    secretInfo: save.secretInfo, playerAction, chapterText, povScene: true, language
  });
  const stateRaw = await callText({ system: statePrompt.system, user: statePrompt.user, kind: 'turn_state', worldId: world.id, saveId, providerOverride });
  const stateParsed = parseModelJSON(stateRaw);
  ...
  const { framedImagePrompt, imageUrl } = await generateTurnImage(world, settings, parsed.image_prompt);
  const { turn, turnNumber } = persistTurn({ saveId, world, playerAction, parsed, framedImagePrompt, imageUrl, itemDefs, allTurns, povCharacter: povCharacterName });
  await maybeSummarize(saveId, turnNumber);
  // The Archivist gets povCharacter too, so it scopes known_by to whoever
  // was actually in this scene instead of defaulting new facts to public
  // the way a normal MC-POV chapter's facts do (see buildArchivistPrompt).
  fireArchivist({ world, saveId, turnNumber, playerAction, chapterText: turn.chapterText, characters: otherCharacters, activeCharacter: mcCharacter, settings, povCharacter: povCharacterName });
  return turn;
}
...
module.exports = {
  ...
  getStoryClock, getActiveMastermindPlan, getCurrentOutfit,
