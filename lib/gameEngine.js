...
// real "last time this was anchored" to diff pacing language against.
// Nothing reads this yet -- populated now purely so Milestone 3 needs no
// schema or call-shape change to start using it.
...
function gatherTurnContext(saveId) {
  ...
  // roles/archivist.js). Splitting them three ways is what keeps a fact like
  ...
  //     still-evolving state, capped (oldest dropped first).
  ...
  //     OTHER CHARACTERS, capped the same way as KNOWN FACTS (see Milestone 1
  //     of the V2 design doc) -- this is the direct fix for the production
  //     bug where a long-lived character's uncapped fact list eventually
  //     pushed the whole prompt past a local model's context window.
  //
  // Sorted by turnNumber (not left in whatever order the query returned)
  // because the Archivist now writes facts asynchronously, off the critical
  // path -- two turns' extraction calls can in principle resolve out of
  // order, so insertion order is no longer guaranteed to match story order
  // the way it did when this was written synchronously inline in persistTurn.
  const allMemoryFacts = db.get('memoryFacts').filter({ saveId }).value().sort((a, b) => a.turnNumber - b.turnNumber);
  ...
  Object.keys(characterFactsByName).forEach(name => {
    characterFactsByName[name] = characterFactsByName[name].slice(-RELEVANT_FACTS_LIMIT);
  });
...
async function maybeSummarize(saveId, turnNumber) {
  ...
  try {
    ...
    if (summaryText) {
      db.get('memoryFacts').push(newMemoryFact({ saveId, turnNumber, fact: `(summary) ${summaryText}`, character: null, type: 'plot' })).write();
    }
