...
// Milestone 3's Mastermind cadence: every N turns, not every turn -- see
// fireMastermind for why it's anchored to turnNumber % N === 1 rather than
// === 0, so a plan gets seeded within the first couple of real turns
// instead of a save having nothing to draw on until turn 5.
const MASTERMIND_EVERY = 5;
...
async function gatherTurnContext(saveId, playerAction) {
  ...
  const { memoryFacts, factsByCharacter } = await retriever.retrieveRelevantFacts({
    saveId, queryText, characterNames: characterRows.map(c => c.name), viewerCharacter: activeCharacter.name, limit: RELEVANT_FACTS_LIMIT, settings
  });
  ...
  // The Mastermind's hidden plan (Milestone 3): read back the same way as
  // storyClock/timelineEvents above, into every normal turn's prompt (see
  // buildTurnContextBlocks's MASTERMIND'S PLAN block). null until the
  // periodic cadence first fires (see fireMastermind) -- buildTurnContextBlocks
  // already treats that as "no block" the same way it does for storyClock.
...
// pattern as fireArchivist/fireProofreader above. Only on the periodic
// cadence (MASTERMIND_EVERY) -- not every turn, per the design doc:
// reviewing the plan against every single scene would be wasteful and would
// churn a plan that's supposed to be a stable throughline, not something
// that reacts to each individual beat.
//
// turnNumber % MASTERMIND_EVERY === 1 (not === 0): for a world without a
// "background" popup, turn 0 is a free, static opening_chapter turn created
// directly in createSave, never through playTurn/playTurnStreaming, so
// turnNumber here effectively starts at 1 for those. For a world WITH one
// (the common case -- see createSave's own comment), turn 0 IS the first
// real playTurn/playTurnStreaming call instead. Either way, anchoring to
// turnNumber % 5 === 1 means the plan seeds on the second real turn at the
// latest, rather than making every save wait until turn 5 with nothing to
// draw on.
//
...
function fireMastermind({ world, saveId, turnNumber, settings }) {
  if (turnNumber % MASTERMIND_EVERY !== 1) return;
